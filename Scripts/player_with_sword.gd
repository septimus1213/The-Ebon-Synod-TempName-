...
@onready var swordattacks: AnimatedSprite2D = $Swordattacks
@onready var playerattacks: AnimatedSprite2D = $playerattacks
...
func _process(_delta: float) -> void:
	...
	if attack and player_with_sword.visible == true:
		...
		var angle = atan2(mouse_pos.y - player_pos.y, mouse_pos.x - player_pos.x)
		...
		if angle > -PI/4 and angle <= PI/4:
			...
			swordattacks.play("attackleft")
			playerattacks.play("attackleft")
